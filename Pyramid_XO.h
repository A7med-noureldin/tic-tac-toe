...
public:
    Player_Pyramid (string name, T symbol);
    void getmove(int& x, int& y);
};

template <typename T>
class RandomPlayer_Pyramid : public RandomPlayer<T>{
public:
    RandomPlayer_Pyramid (T symbol);
    void getmove(int &x, int &y);
};
...
template<typename T>
bool Board_Pyramid<T>::update_board(int x, int y, T symbol) {
...
template<typename T>
void Board_Pyramid<T>::display_board() {
    bool good = true;
    cout << endl << setw(19);
    for (int i = 0; i < 3; i++) {
        if (i == 1)
...
template<typename T>
bool Board_Pyramid<T>::is_draw() {
    return (this->n_moves == 9 && !is_win());
}

template<typename T>
bool Board_Pyramid<T>::is_win() {
    //check Rows
...
template<typename T>
bool Board_Pyramid<T>::game_is_over() {
...
template<typename T>
Player_Pyramid<T>::Player_Pyramid(string name, T symbol):Player<T>(name, symbol) {}

template<typename T>
void Player_Pyramid<T>::getmove(int &x, int &y) {
...
template<typename T>
RandomPlayer_Pyramid<T>::RandomPlayer_Pyramid(T symbol):RandomPlayer<T>(symbol) {
    this->name = "Random Computer Player";
    srand(static_cast<unsigned int>(time(0)));
}

template<typename T>
void RandomPlayer_Pyramid<T>::getmove(int &x, int &y) {
    x = rand() % 3;  //Rows = 3
    y = rand() % 5; // Columns = 5
}

#endif //GAMETEST_PYRAMID_Pyramid_H
